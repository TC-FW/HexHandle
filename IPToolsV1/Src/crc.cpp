...
uint8_t Crc8_Basic(uint8_t forworddata, uint8_t newdata)
{
	...
	for (i = 8u; i > 0u; --i)
	{
		if (forworddata & 0x80u)
		{
			forworddata = (uint8_t)(forworddata << 1u) ^ 0x07u;
		}
		else
		{
			forworddata = (uint8_t)(forworddata << 1u);
		}
	}
	return forworddata;
}
...
uint16_t Crc16(uint8_t* ptr, uint16_t len)
{
	...
	else
	{
		do
		{
			...
			for (i = 0U; i < 8U; i++)
			{
				if ((tmp & 0x0001U) ^ (tmpData & 0x0001U))
				{
					tmp = (tmp >> 1U) ^ POLY;
				}
				else
...
uint32_t Crc32_Basic(uint32_t forworddata, uint32_t newdata, uint8_t ending)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (ending == DATA_LITTLEENDING)
		{
			temp = (uint8_t)(newdata >> (24 - 8 * i));
		}
		else
		{
			temp = (uint8_t)(newdata >>  (8 * i));
		}
		forworddata = CRC32Table[(forworddata ^ temp) & 0xff] ^ (forworddata >> 8);
	}
	return forworddata ^ ~0U;
}
...
uint32_t Crc32_MPEG2(uint32_t forworddata, uint32_t newdata, uint8_t ending)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		forworddata = (forworddata << 8) ^ crc_mpeg2_table[(forworddata >> 24 ^ temp) & 0xff];
	}
	return forworddata;
}
